...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        eframe::egui::TopBottomPanel::top("editor_mode_switch").show(ctx, |ui| {
    ui.horizontal(|ui| {
        ui.selectable_value(&mut self.mode, EditorMode::Video, "Video Editor");
        ui.selectable_value(&mut self.mode, EditorMode::Photo, "Photo Editor");
    });
});
        ...
        if self.mode == EditorMode::Photo {
    photo_editor::show_photo_editor(ctx, &mut self.photo_doc);
    return;
}
